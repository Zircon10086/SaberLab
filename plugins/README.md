...
| 插件 | 说明 | 许可证 | 放置方式 |
|---|---|---|---|
| `chro/` | 3D 回放组件（Local-ChroViewer 构建产物） | GPL-2.0-only（外部独立项目） | 将 Local-ChroViewer 的 `dist/` 构建产物放入本目录（含其 LICENSE） |
...
- 源码**不在本仓库**（许可证边界）：构建于独立项目
  `../Local-ChroViewer`（GPL-2.0-only，ChroViewer 移植），此处仅存放**构建产物**
